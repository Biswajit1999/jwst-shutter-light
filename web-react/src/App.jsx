...
export default function App() {
  ...
  const isSmallTrialDemo = runLabel === 'demo' || runLabel === 'custom' || summary.data?.data_kind === 'synthetic_smoke_test';
      ...
      <div className="mx-auto max-w-7xl px-5 py-10">
        <header className="mb-8 rounded-3xl border border-slate-800 bg-slate-950/80 p-7 backdrop-blur">
          ...
          <div className="mt-6 flex flex-wrap gap-3 text-sm">
            ...
            {summary.data && (
              <span className={`${pill} ${isSmallTrialDemo ? 'border-amber-800 bg-amber-950/40 text-amber-200' : 'border-emerald-800 bg-emerald-950/40 text-emerald-200'}`}>
        ...
        {isSmallTrialDemo && (
          <div className="mb-6 flex items-start gap-2 rounded-xl border border-amber-900 bg-amber-950/30 p-4 text-sm text-amber-200">
            ...
            The metrics and figures below were generated at a small Monte Carlo trial count
            (scripts/run_analysis.py --demo), for fast iteration and CI smoke-testing. The full
            production-scale Monte Carlo run (scripts/run_analysis.py, {p.productionTrials} trials)
            replaces these once executed. All results, at any trial count, are synthetic Monte Carlo
            output built from verified real instrument parameters (see data/manifest.csv) — never real
            NIRSpec telemetry.
          </div>
        )}
        ...
        <section className="mt-6 grid gap-6 md:grid-cols-2">
          <Section icon={Download} title="Downloads and provenance manifest">
            ...
            <p className="mt-4 text-xs text-slate-500">
              ...
              selection_reason and licence_or_terms for every verified instrument parameter used in the Monte Carlo model.
            </p>
          </Section>
